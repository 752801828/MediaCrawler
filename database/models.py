...
#
# The local storage schema keeps the fields returned by each platform.  The
...
class WeiboNote(Base):
    ...
    nickname = Column(Text, comment='用户昵称')
    avatar = Column(Text, comment='用户头像')
    gender = Column(Text, comment='用户性别')
...
class TiebaNote(Base):
    ...
    user_link = Column(Text, comment='用户主页')
    creator_hash = Column(String(64), index=True, comment='创作者兼容哈希')
...
class BilibiliUpInfo(Base):
    ...
    total_liked = Column(BigInteger, comment='获赞数')
...
class BilibiliContactInfo(Base):
    ...
    id = Column(Integer, primary_key=True, comment='主键ID')
    up_id = Column(String(64), index=True, comment='UP主ID')
    fan_id = Column(String(64), index=True, comment='粉丝ID')
...
class DyCreator(Base):
    ...
    videos_count = Column(Text, comment='作品数')
...
class XhsCreator(Base):
    ...
    fans = Column(Text, comment='粉丝数')
    interaction = Column(Text, comment='互动数')
    tag_list = Column(Text, comment='标签')


class KuaishouCreator(Base):
    __tablename__ = 'kuaishou_creator'
    id = Column(Integer, primary_key=True, comment='主键ID')
    user_id = Column(String(64), index=True, comment='用户ID')
    nickname = Column(Text, comment='用户昵称')
    avatar = Column(Text, comment='用户头像')
...
class ZhihuCreator(Base):
    ...
    gender = Column(Text, comment='用户性别')
    ip_location = Column(Text, comment='IP归属地')
    follows = Column(Integer, comment='关注数')
    fans = Column(Integer, comment='粉丝数')
    anwser_count = Column(Integer, comment='回答数')
    video_count = Column(Integer, comment='视频数')
    question_count = Column(Integer, comment='提问数')
    article_count = Column(Integer, comment='文章数')
    column_count = Column(Integer, comment='专栏数')
    get_voteup_count = Column(Integer, comment='获赞数')
    add_ts = Column(BigInteger, comment='添加时间戳')
    last_modify_ts = Column(BigInteger, comment='最后修改时间戳')
